...
export default function SearchBarWithSuggestions({ onSearch }: SearchBarProps) {
                ...
                {isOpen && (
                    <CommandList className="absolute top-full mt-2 w-full bg-white rounded-md shadow-lg z-50 border">
                        ...
                        {!isLoading && suggestions.length === 0 && debouncedSearchTerm && (
                            <CommandEmpty>
                                <div className="p-4 text-sm">No results found for "{debouncedSearchTerm}".</div>
                            </CommandEmpty>
                        )}
                        
                        {!isLoading && suggestions.map((provider) => {
                            const matchedService = provider.services && provider.services[0];
                            const itemValue = `${provider.healthcare_name} ${matchedService ? matchedService.name : provider.address}`;
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex flex-col min-w-0">
                                        <div className="font-medium truncate">{provider.healthcare_name}</div>
