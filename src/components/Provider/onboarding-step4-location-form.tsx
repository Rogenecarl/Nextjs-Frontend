...
import { ChevronLeft, ChevronRight, Info, Loader2, MapPin, Navigation } from 'lucide-react';
...
const onboardingLocationFormSchema = OnboardingCreateProviderProfileFormSchema.pick({
    province: true,
    city: true,
    address: true,
    longitude: true,
...
export default function OnboardingStep4LocationForm() {
    ...
    const setData = useOnboardingCreateProviderProfileStore((state) => state.setData);
    ...
    const handleLocationSelect = (lng: number, lat: number, shouldAnimate: boolean = true) => {
    ...
    const reverseGeocode = async (lng: number, lat: number) => {
        try {
            ...
            if (data.features && data.features.length > 0) {
                ...
                const province = context.find((c: any) => c.id.includes('region'))?.text || '';
    ...
    return (
        <div className="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="mb-8 text-center">
                ...
                <h1 className="mb-2 text-2xl font-bold sm:text-3xl">Business Location</h1>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                        ...
                        <div className="space-y-8">
                                        ...
                                        <div className="space-y-2 text-sm">
                                            {selectedLocation.address && (
                                                <div className="flex justify-between">
                                                    ...
                                                    <span className="text-blue-800 text-right flex-1 ml-2">{selectedLocation.address}</span>
                                                </div>
                                            )}
                                            {selectedLocation.city && (
                                                <div className="flex justify-between">
                                                    <span className="text-blue-700 font-medium">City:</span>
                                                    <span className="text-blue-800 text-right flex-1 ml-2">{selectedLocation.city}</span>
                                                </div>
                                            )}
                                            {selectedLocation.province && (
                                                <div className="flex justify-between">
                                                    <span className="text-blue-700 font-medium">Province:</span>
                                                    <span className="text-blue-800 text-right flex-1 ml-2">{selectedLocation.province}</span>
                                                </div>
                                            )}
                                            <div className="flex justify-between">
                                                <span className="text-blue-700 font-medium">Latitude:</span>
                                                <span className="text-blue-800 font-mono">{selectedLocation.lat.toFixed(6)}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-blue-700 font-medium">Longitude:</span>
                                                <span className="text-blue-800 font-mono">{selectedLocation.lng.toFixed(6)}</span>
                                            </div>
                                        </div>
                                    </div>
                                )}
                            ...
                            <Alert className="border-blue-200 bg-blue-50">
                                ...
                                <AlertDescription className="text-blue-800">
                                    <strong>Tip:</strong> Click anywhere on the map to set your location, or use the "Use Current Location" button
                                    to automatically detect your position. You can drag the marker to fine-tune your location.
                                </AlertDescription>
                            </Alert>
                        </div>
                    </CardContent>
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:justify-between">
                    ...
                    <Button
                        ...
                        disabled={!selectedLocation || !!errors.latitude || !!errors.longitude}
                    >
                        Next: Summary
