...
export function MonthView({ currentDate, events, appointments = [] }: MonthViewProps) {
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
      ...
      <div className="grid grid-cols-7 border-b border-gray-200">
        {weekDays.map((day) => (
          <div key={day} className="px-4 py-3 text-sm font-medium text-gray-600 text-center bg-gray-50">
            {day}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7">
        {days.map((day, index) => {
          ...
          const allItems = [
            ...
            ...dayAppointments.map(apt => ({ type: 'appointment', data: apt }))
          ]
